...
reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows Photo Viewer\Capabilities\FileAssociations" /f /t REG_SZ /d "PhotoViewer.FileAssoc.Tiff" >nul

REM Set Windows Photo Viewer as the default app for desired extensions
for %%x in (%extensions%) do (
    echo Setting default app association for %%x...
    assoc %%x=PhotoViewer.FileAssoc.Tiff
    reg add "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows Photo Viewer\Capabilities\FileAssociations" /f /v "%%x" /t REG_SZ /d "PhotoViewer.FileAssoc.Tiff" >nul
)
